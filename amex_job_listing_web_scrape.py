...
# Configure email distribution settings
email_list = ['jacob.shughrue@gmail.com']
msg = MIMEMultipart()
msg['Subject'] = "Open Positions @ Amex"
msg['From'] = 'jacob.shughrue@gmail.com'
server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
smtp_key = os.environ.get('gmail_smtp')
server.login("jacob.shughrue@gmail.com", smtp_key)
...
position_search_term = 'SENIOR%20ANALYST'  # 'Data%20Science' # where %20 is a space " " in an encoded URL
position_location = 'New%20York'

# set site address
site = 'https://aexp.eightfold.ai/careers?' + 'query=' + position_search_term + '&location=' + position_location

# open site in browser - allows Selenium to interact with the webpage
...
# create a list of all html labels for all position cards on the page
...
for line in range(job_titles_length):
    try:
        ...
        # grab the url of the current job description
        link = driver.current_url

        # grab the url of the selected position
        link_list.append(link)

        # scrape bulleted points from each posting
        ...
        description_list.append('Error: position html no longer present')
        link_list.append('Error: position html no longer present')
        print('Error: position html no longer present')
        continue

# create a new df with our job descriptions and urls stored
...
df_points = pd.DataFrame(bullet_list).stack().groupby(level=-1).agg(list)

# combine our three separate tables into one cohesive df
df = pd.concat([df_job_titles, df_descriptions, df_points, df_links], axis=1)
df.columns = ['title', 'description', 'points', 'link']
...
search_words = {'data', 'finance', 'financial' 'dashboard', 'dashboards', 'powerbi', 'bi', 'tableau', 'analysis',
                'analyze', 'analytical', 'analytics', 'deploy', 'deploying', 'trend', 'trends', 'science', 'scientist',
                'decision', 'ai', 'ml', 'aiml', 'learning', 'sql', 'python', 'predictive', 'modeling', 'processing',
                'manipulation', 'spark', 'pyspark', 'programming', 'findings', 'insights',
                'quantitative'}  # case sensitive
...
df['points'] = df['points'].replace('\\\\n', ' \n', regex=True).str[2:].str[:-2]

# select columns for export
df = df[['title', 'key_word_count', 'all_key_words', 'min_salary', 'max_salary', 'salary_midpoint', 'link']]
...
# send df results as html table via email
html = """\
<html>
  <head></head>
  <body>
